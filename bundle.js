...
async function loadBoard(boardId) {
  ...
  for (const t of tracks) {
    ...
    // Lazily decode: skip long-track pre-decoding here to avoid huge memory
    // spikes when a board holds several long files (e.g. a 30-min halftime
    // track can be ~600MB once decoded). Actual decode now happens on tap,
    // in tapTile().
  }
}
...
async function tapTile(t) {
  ...
  if (!getBuffer(t.id)) {
    ...
    if (!rec) { toast('No audio — tap ⚙ → Replace Audio'); return; }
    toast('Loading…');
    try { await decodeAudio(t.id, rec.blob); }
    catch (e) { toast('Could not load audio'); return; }
  }
  stopAll(0);
...
function onTrackEnd(trackId) {
  ...
  if (t && t.autoPlayNext) {
    ...
    if (next && next.type !== 'label') { tapTile(next); return; }
  }
  render();
}
